    ...
    <section className="px-8 md:px-14 py-12 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-dashed border-white/10 rounded-3xl bg-white/[0.02]">
          ...
          <p className="text-base font-bold text-white">Kitchens Onboarding in Progress</p>
          <p className="text-xs text-gray-400 mt-1 max-w-md mx-auto">
            Dishes created by approved restaurant partners will appear here dynamically.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {products.map((p) => (
            <div
              key={p._id}
              className={`bg-[#111] rounded-2xl overflow-hidden border transition group ${
                p.isAvailable
              ...
              <div className="relative aspect-square overflow-hidden">
                <Image
                  ...
                  className={`object-cover transition duration-500 ${
                    p.isAvailable ? "group-hover:scale-105" : "grayscale-[40%]"
                  }`}
                />
                {!p.isAvailable && (
                  <div className="absolute inset-0 bg-black/60 backdrop-blur-[1px] flex items-center justify-center z-10 pointer-events-none">
                    <span className="text-[10px] font-black uppercase tracking-wider bg-red-600/90 text-white px-2 py-0.5 rounded shadow border border-red-400/30">
                      Out of Stock
                    </span>
                  </div>
                )}
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${p.type === "veg" ? "bg-green-400" : "bg-red-400"}`}
                  />
                </span>
              </div>
              <div className="p-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <button
                    ...
                    title={p.isAvailable ? "Add to cart" : "Out of Stock"}
                    className="w-7 h-7 rounded-full bg-orange-500 hover:bg-orange-600 disabled:bg-white/10 disabled:text-white/20 disabled:cursor-not-allowed transition flex items-center justify-center text-white font-bold text-lg leading-none"
                  >
                    +
                  </button>
                </div>
